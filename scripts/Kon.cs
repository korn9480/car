...
public class Kon : KinematicBody2D
{
    [Export]
    ...
    {
        GD.Print("kon");   
        // var main = GetNode<Node2D>("Node2D");
        // main.Connect("get_control",this,nameof(get_input));
    }
    
    public void get_input(string star){
        if (true){
            int turn = 0;
            if (Input.IsActionPressed("kon_right")){
                turn += 1;
            }
            if (Input.IsActionPressed("kon_letf")){
                turn -= 1;
            }
            steer_angle = turn * Mathf.Deg2Rad(steering_angle);
            velocity = Vector2.Zero;
            if (Input.IsActionPressed("kon_up")){
